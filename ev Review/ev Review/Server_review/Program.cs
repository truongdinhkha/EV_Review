...
namespace Server_review
{
    class Server
    {
       ...
       string ConnectionString = @"Data Source=DINH-KHA\SQLEXPRESS;Initial Catalog=model;Integrated Security=True";
        ...
        private void HandleClient(TcpClient client)
        {
            try
            {
                ...
                else if (requestType == "Chat_user")
                {
                    ...
                    //response = Chat(requestParts[1], requestParts[2]);
        ...
        private void handle(TcpClient client, string endPoint)
        {
            NetworkStream stream = client.GetStream();
            byte[] buffer = new byte[1024];
        ...
        private bool new_post(string Name_Company,string title,string content,int Star,string ID_Nguoi_Dung)
        {
            try
            {
                using (SqlConnection connection = new SqlConnection(ConnectionString))
                {
                    ...
                    string sqlstring = $"Insert into Post(Name_Company,Title,Content,Star,Date,Name_Nguoi_Viet) values('{Name_Company}',' {title} ','{content}', {Star} ,'{t}', '{ID_Nguoi_Dung}' )";
        ...
        private string Chat(string name, string tinnhan)
        {
            
                //TcpListener serverListener1 = new TcpListener(IPAddress.Any, port);
                //serverListener1.Start();
                //Console.WriteLine("Đã bắt đầu nghe tại port" + port);
            string responseMessage = $"{name}: {tinnhan}";
